...
def main():
    ...
    layout = [[sg.Text('PyInstaller EXE Creator', font='Any 15')],
              [sg.Text('Source Python File'), sg.Input(default_text=os.path.join(working_dir, 'barcode_auto.py'), key='-sourcefile-', size=(45, 1)),
               sg.FileBrowse(file_types=(("Python Files", "*.py"),))],
              [sg.Text('Icon File'), sg.Input(default_text=os.path.join(working_dir, 'icon.ico'), key='-iconfile-', size=(45, 1)),
    ...
    while True:
        ...
        command_line = 'pyinstaller -Fw --clean "{}" {} {} {} {}'.format(source_file, icon_option, workpath_option, dispath_option, specpath_option)
        if event == 'Make EXE':
            try:
                print(command_line)
